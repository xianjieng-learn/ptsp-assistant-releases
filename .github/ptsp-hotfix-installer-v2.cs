...
internal static class Program
{
    ...
    {
        bool silent = args.Any(delegate(string a)
        {
            return string.Equals(a, "--silent", StringComparison.OrdinalIgnoreCase)
    ...
    private static void PreparePayloadVersion(string payloadDir)
    {
        ...
        if (File.Exists(backgroundPath))
        {
            ...
            background = background.Replace("3.0.11", TargetVersion);
        ...
        if (File.Exists(manifestPath))
        {
            ...
            manifest = manifest.Replace("\"version\": \"3.0.11\"", "\"version\": \"" + TargetVersion + "\"");
    ...
    private static string FindExtensionPath(string root)
    {
        ...
        candidates.Add(Path.Combine(root, "Extension"));
        candidates.Add(Path.Combine(root, "extension"));

        if (Directory.Exists(root))
        {
            candidates.AddRange(Directory.GetDirectories(root, "Extension-v*").OrderByDescending(delegate(string p) { return Directory.GetLastWriteTimeUtc(p); }));
            candidates.AddRange(Directory.GetDirectories(root, "extension-v*").OrderByDescending(delegate(string p) { return Directory.GetLastWriteTimeUtc(p); }));
        }
    ...
    {
        return (value ?? "0").Split('.').Select(delegate(string part)
        {
            int n;
            return int.TryParse(part, out n) ? n : 0;
        }).ToArray();
    }
